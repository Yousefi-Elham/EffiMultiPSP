...
#testset1<- datAbbVieALL.wide %>% dplyr::select(ends_with(".1"|ends_with(".18")))
#dattestxxx<- datAbbViepull.wideC %>% filter(VISIT %in% c(1,18))
#testset2 <- testset1[complete.cases(testset1),] %>% 
#  relocate(ends_with(".1"), .after = last_col())

#########################################################################
##                         Rescoring AbbVie (wide) data from two visits

#########################################################################

datAbbVie.118 <- datAbbVie %>% filter(VISIT %in% c(1,18)) 


### Rescoring the items based on the FDA recommendation. Note that items 3 and 13, 
### recoded as 1 and 5 in implementations, don't contain any changes.


datAbbVie.118.RESC <- datAbbVie.118 %>%
  mutate( 
    DV = case_when( 
      ITEM==1& DV == 0 ~ 0,
      ITEM==1& DV == 1 ~ 1,
...
dfall1 <- dplyr::filter(datAbbVie.118.RESC,TRT==1) %>%
  arrange(ITEM)
...
dfall2 <- dplyr::filter(datAbbVie.118.RESC,TRT==2) %>%
  arrange(ITEM)
...
dfall3 <- dplyr::filter(datAbbVie.118.RESC,TRT==3) %>%
  arrange(ITEM)
...
ANCOVA.mean.est.RESC.13 <- round(Test13$coef.trt.RESC,digits=3)
ANCOVA.SE.est.RESC.13 <- round(Test13$se.trt.RESC,digits=3)
pval.item.RESC.13 <- round(Test13$pval.eachij.RESC,digits = 3)
...
ANCOVA.mean.est.RESC.23 <- round(Test23$coef.trt.RESC,digits=3)
ANCOVA.SE.est.RESC.23 <- round(Test23$se.trt.RESC,digits = 3)
pval.item.RESC.23 <- round(Test23$pval.eachij.RESC,digits = 3)
...
df.des.RESC <- data.frame(matrix(nrow = 3*nitem, ncol = 9))
colnames(df.des.RESC) <- c("Baseline.Mean", "Baseline.SE", "Week 52.Mean", "Week 52.SE", 
...
for (i in 1:nitem) {
  ...
  df.des.RESC[order_sequence[i], ] <- c(Basemean.trt1[i], BaseSE.trt1[i],W52mean.trt1[i], W52SE.trt1[i],
                                   Difmean.trt1[i], DifSE.trt1[i],ANCOVA.mean.est.RESC.13[i],ANCOVA.SE.est.RESC.13[i],pval.item.RESC.13[i])
  
  
  df.des.RESC[order_sequence[i]+1, ] <- c(Basemean.trt2[i], BaseSE.trt2[i],W52mean.trt2[i], W52SE.trt2[i],
                                     Difmean.trt2[i], DifSE.trt2[i],ANCOVA.mean.est.RESC.23[i],ANCOVA.SE.est.RESC.23[i],pval.item.RESC.23[i])
  
  
  df.des.RESC[order_sequence[i]+2, ] <- c(Basemean.trt3[i], BaseSE.trt3[i],W52mean.trt3[i], W52SE.trt3[i],
...
format_means.Base <- sprintf("%.3f", df.des.RESC[, "Baseline.Mean"] )
format_SE.Base <- sprintf( "(%s)" , df.des.RESC[, "Baseline.SE"])

format_val.Base <- paste0(format_means.Base,format_SE.Base)

###

format_means.W52 <- sprintf("%.3f", df.des.RESC[, "Week 52.Mean"])
format_SE.W52 <- sprintf("(%s)", df.des.RESC[, "Week 52.SE"])

format_val.W52<- paste0(format_means.W52,format_SE.W52)

###

format_means.Diff <- sprintf("%.3f" , df.des.RESC[, "Difference.Mean"])
format_SE.Diff <- sprintf("(%s)", df.des.RESC[, "Difference.SE"])

format_val.Diff <- paste0(format_means.Diff,format_SE.Diff)

###

format_means.ANCOVA <- sprintf("%.3f", df.des.RESC[, "ANCOVA.Mean"])
format_SE.ANCOVA <- sprintf("(%s)", df.des.RESC[, "ANCOVA.SE"])

format_val.ANCOVA <- paste0(format_means.ANCOVA,format_SE.ANCOVA)

###

df.formatted.RESC <- data.frame("Baseline" = format_val.Base, "Week 52" = format_val.W52, "Difference" = format_val.Diff,
                           "ANCOVA" = format_val.ANCOVA , "P-value"= df.des.RESC[, "P-value"])



df.formatted.RESC <- df.formatted.RESC %>% mutate_all(~ifelse(. %in% c("NA", "NA(NA)", NA), "-", .))


# Add row names
row.names.df <- paste0(c("Til. 2000 mg-", "Til. 4000 mg-", "Placebo-"), rep(1:nitem, each=3))
rownames(df.formatted.RESC) <- row.names.df


print(df.formatted.RESC)
View(df.formatted.RESC)
