...
################################################################################
### Estimation of the pooled covariance matrix based on the three treatment
### groups, for the purpose of data generation
################################################################################

### Take the subset of the ABBV-8E12 trial at two visits of interest and the first treatment group (tilavonemab 2000)

df1 <- filter(datAbbVie,VISIT%in%c(1,18)&TRT==1) %>%
  arrange(ITEM) %>%
  group_by(ITEM)

nitemPL <- 2*length(unique(df1$ITEM))


### Complete item-level scores from the wide format of the data

df1.piw <- pivot_wider(df1,id_cols=c(ID,STUDY_ID,TRT,SCALE), names_from = c(ITEM,VISIT),values_from = DV,names_prefix = "ITEMV",names_sep = ".")

df1.piwFUL <- df1.piw[complete.cases(df1.piw),]

df1.piwFUL <- df1.piwFUL %>% 
  relocate(ends_with(".1"), .after = last_col())


### sample size of the first treatment group
n1 <- nrow(df1.piwFUL)

df1.fc.piwSS <- df1.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  mutate(SumScore=rowSums(.)) 

itemName <- colnames(df1.piwFUL)[5:ncol(df1.piwFUL)] # only the item variables


### estimation of the Covariance matrix for the first treatment group
ITEMcov1 <- df1.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  do(as.data.frame(cov(., method="pearson")))


### estimation of the mean vector for the first treatment group
ITEMmean1 <- df1.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  colMeans()
...
df2 <- filter(datAbbVie,VISIT%in%c(1,18)&TRT==2) %>%
  arrange(ITEM) %>%
  group_by(ITEM)
...
df2.piw <- pivot_wider(df2,id_cols=c(ID,STUDY_ID,TRT,SCALE), names_from = c(ITEM,VISIT),values_from = DV,names_prefix = "ITEMV",names_sep = ".")

df2.piwFUL <- df2.piw[complete.cases(df2.piw),]

df2.piwFUL <- df2.piwFUL %>% 
  relocate(ends_with(".1"), .after = last_col())



### sample size of the second treatment group
n2 <- nrow(df2.piwFUL)

df2.fc.piwSS <- df2.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  mutate(SumScore=rowSums(.))


### estimation of the Covariance matrix for the second treatment group
ITEMcov2 <- df2.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  do(as.data.frame(cov(., method="pearson")))


### estimation of the mean vector for the second treatment group
ITEMmean2 <- df2.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  colMeans()
...
df3 <- filter(datAbbVie,VISIT%in%c(1,18)&TRT==3) %>%
  arrange(ITEM) %>%
  group_by(ITEM)
...
df3.piw <- pivot_wider(df3,id_cols=c(ID,STUDY_ID,TRT,SCALE), names_from = c(ITEM,VISIT),values_from = DV,names_prefix = "ITEMV",names_sep = ".")

df3.piwFUL <- df3.piw[complete.cases(df3.piw),]

df3.piwFUL <- df3.piwFUL %>% 
  relocate(ends_with(".1"), .after = last_col())



### sample size of the third treatment group
n3 <- nrow(df3.piwFUL)


df3.fc.piwSS <- df3.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  mutate(SumScore=rowSums(.))


### estimation of the Covariance matrix for the third treatment group
ITEMcov3 <- df3.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  do(as.data.frame(cov(., method="pearson")))


### estimation of the mean vector for the third treatment group
ITEMmean3 <- df3.piwFUL %>%
  dplyr::select(starts_with("ITEMV")) %>%
  colMeans()

################################################################################

### Pooled covariance and correlation matrices


### Pooled Covariance matrix
ITEMcov.pool <- ((n1-1)*ITEMcov1 +(n2-1)*ITEMcov2 + (n3-1)*ITEMcov3)/(n1+n2+n3-3)

### correlation matrix
ITEMsigma.pool <- as.vector(diag(as.matrix(ITEMcov.pool)))
ITEMcor.pool <- solve(sqrt(diag(ITEMsigma.pool,nitemPL,nitemPL)))%*%as.matrix(ITEMcov.pool)%*%solve(sqrt(diag(ITEMsigma.pool,nitemPL,nitemPL)))
row.names(ITEMcor.pool) <- colnames(ITEMcor.pool) <- itemName

### An alternative way to calculate the correlation matrix
ITEMcor1.pool <- cov2cor(as.matrix(ITEMcov.pool))

all.equal(ITEMcor1.pool,ITEMcor.pool)# both are equal
...
delta0 <- c(rep(0.2,nitem),rep(0,nitem))
delta1 <- c(rep(0.25,nitem),rep(0,nitem))
delta1p <- c(rep(0.3,nitem),rep(0,nitem))
delta2 <- c(0.1273841, 0.2601574, 0.3508167, 0.2327014, 0.2287162, 0.2470233, 0.2800658, 0.2331958, 0.2709495, 0.2689898,rep(0,nitem))
delta3 <- c(rep(0.85,3),rep(0,7),rep(0,nitem)) #History domain=1,rest=0
delta4 <- c(rep(0,3),rep(1.25,2),rep(0,5),rep(0,nitem)) #Bulbar exam domain=1,rest=0
delta5 <- c(rep(0,5),rep(0.5,5),rep(0,nitem)) #Gait/midline exam domain=1,rest=0
delta6 <- c(rep(0.5,5),rep(0,5),rep(0,nitem)) #History,Bulbar exam domain=1,rest=0
delta7 <- c(rep(0.3,3),rep(0,2),rep(0.3,5),rep(0,nitem)) #History,Gait/midline domain=1,rest=0
delta8 <- c(rep(0,3),rep(0.35,7),rep(0,nitem)) #Bulbar,Gait/midline domain=1,rest=0
delta9 <- c(2.5,rep(0,9),rep(0,nitem))
delta10 <- c(0,0,0,2.5,rep(0,6),rep(0,nitem))
delta11 <- c(rep(0,5),2.5,rep(0,4),rep(0,nitem))


Dvec.all <- rbind(d0=delta0,d1=delta1,d1p=delta1p,d2=delta2,d3=delta3,d4=delta4,d5=delta5,d6=delta6,d7=delta7,
                  d8=delta8,d9=delta9,d10=delta10,d11=delta11)
Dvec.all
...
# This function first generates the data under the null and the alternative from the multivariate normal distribution
# It then applies the treatment effects (under the alternative) and also uses the FDA scoring to the generated data.
...
TEST.IRIfun <- function(ni,nj,Tmeani,Tcovi,Tmeanj,Tcovj,itemName=itemName,
  ...
  ### First group (test group) data generation
  DF.dgeni <- rmvnorm(ni,mean = Tmeani,sigma = as.matrix(Tcovi))
  
  DF.dgendisi <- data.frame(matrix(NA,nrow = nrow(DF.dgeni), ncol = ncol(DF.dgeni)))
  for (tt in 1:ncol(DF.dgeni)) {
    DF.dgendisi[,tt] <- pmin(pmax(0,round(DF.dgeni[,tt])),4)
    
  }
  
  DF.dgeniT <- DF.dgendisi %>%
    mutate(TRT.NEW= rep(2,nrow(DF.dgendisi)))
  
  
  ### second group (control group) data generation
  DF.dgenj <- rmvnorm(nj,mean = Tmeanj,sigma = as.matrix(Tcovj))
  
  DF.dgendisj <- data.frame(matrix(NA,nrow = nrow(DF.dgenj), ncol = ncol(DF.dgenj)))
  for (tt in 1:ncol(DF.dgenj)) {
    DF.dgendisj[,tt] <- pmin(pmax(0,round(DF.dgenj[,tt])),4)
    
  }
  
  DF.dgenjT <- DF.dgendisj %>%
    mutate(TRT.NEW = rep(1,nrow(DF.dgendisj)))
  
  
  ### Combining generated data of both groups
  dat2.piw <- rbind(DF.dgeniT,DF.dgenjT)
  colnames(dat2.piw)[1:20] <- itemName
  ...
  ### Rescoring the generated item scores, at baseline and week 52, based on the FDA recommendation.
...
POWsimSS_list <- future_map(Dvec.all.list, function(Difvec) {
  ...
  future_map(1:n.sim2, function(i) {
    ...
    TEST.IRIfun(ni = nmean, nj = nmean, Tmeani = ITEMmean3-Difvec, Tcovi = ITEMcov.pool,Tmeanj=ITEMmean3,Tcovj=ITEMcov.pool,itemName=itemName,
                csStat.H0=csStat.H0,csStat.H03dom=csStat.H03dom,
                IRTmodelAbb.pullC = IRTmodelAbb.pullC,
                Linmodelpull.CPSIf = Linmodelpull.CPSIf,
                Linmodelpull.CPSIpf = Linmodelpull.CPSIpf,
